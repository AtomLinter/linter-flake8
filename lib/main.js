...
const generateInvalidPointTrace = async (execPath, match, filePath, textEditor, point) => {
  ...
  return {
    severity: 'error',
    description: [
      'ERROR: Flake8 provided an invalid point! See below the details. ',
      `<a href="${newIssueURL}">Report this</a>`, '', '',
      `Original message: ${match[3]} — ${match[5]}`, '', '',
      `Requested point: ${point[0]}:${point[1]}`,
    ].join('\n'),
    location: {
      file: filePath,
...
const determineExecVersion = async (execPath) => {
  let versionString;
  try {
    versionString = await helpers.exec(execPath, ['--version'], { ignoreExitCode: true });
  } catch (e) {
    console.error(e);
    versionString = await helpers.exec(execPath, ['--version'], { stream: 'stderr' });
  }
      ...
      lint: async (textEditor) => {
        ...
        while (match !== null) {
          ...
          try {
            ...
            const point = [line + 1, col + 1];
            const m = generateInvalidPointTrace(execPath, match, filePath, textEditor, point);
            messages.push(m);
          }
