...
const generateInvalidPointTrace = async (execPath, match, filePath, textEditor, line, col) => {
        ...
        while (match !== null) {
          ...
          try {
            messages.push({
              ...
              location: {
                file: filePath,
                position: helpers.generateRange(textEditor, line - 1, col - 1),
              },
            });
          } catch (point) {
            // generateRange encountered an invalid point
            const m = generateInvalidPointTrace(execPath, match, filePath, textEditor, line, col);
            messages.push(m);
          }
