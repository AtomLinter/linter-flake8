...
describe('The flake8 provider for Linter', () => {
  ...
  describe('Substitution check', () => {
    const applyFunc = require('../lib/utils.js').replaceProjectVariable;
    const testPath = '/User/hyamatan/test';
    const testName = path.basename(testPath);
    let pathDataProjectBased;
    let pathDataProjectNameBased;
    let correctProjectBased;
    let correctProjectNameBased;
    beforeEach(() => {
      pathDataProjectBased = [
        '$PROJECT/venv', '$PROJECT/env', '/home/hyamatan/test/venv',
        '$project/../env', '$pRoJeCt/path/venv', '$PrOjEcT/$PROJECT',
      ].join(';');
      pathDataProjectNameBased = [
        '$PROJECT_NAME/venv', '$PROJECT_NAME/env', '/home/hyamatan/test/venv',
        '$project_name/../env', '$pRoJeCt_nAmE/path/venv',
        '$PrOjEcT_nAmE/$PROJECT_NAME',
      ].join(';');
      correctProjectBased = [
        `${testPath}/venv`, `${testPath}/env`, '/home/hyamatan/test/venv',
        `${testPath}/../env`, `${testPath}/path/venv`,
        `${testPath}/${testPath}`,
      ].join(';');
      correctProjectNameBased = [
        `${testName}/venv`, `${testName}/env`, '/home/hyamatan/test/venv',
        `${testName}/../env`, `${testName}/path/venv`,
        `${testName}/${testName}`,
      ].join(';');
    });
    it('The function should replace multiple $PROJECT', () => {
      expect(applyFunc(
        pathDataProjectBased, testPath,
      )).toEqual(correctProjectBased);
    });
    it('The function should replace multiple $PROJECT_NAME', () => {
      expect(applyFunc(
        pathDataProjectNameBased, testPath,
      )).toEqual(correctProjectNameBased);
    });
    it('The function should replace multiple $PROJECT AND $PROJECT_NAME', () => {
      const pathdata = [pathDataProjectBased, pathDataProjectNameBased].concat(
        '$PROJECT/$PROJECT_NAME', '$project/$project_name',
        '$pRoJeCt/$pRoJeCt_NaMe', '$PrOjEcT/$PrOjEcT_nAmE',
      ).join(';');
      const correct = [correctProjectBased, correctProjectNameBased].concat(
        `${testPath}/${testName}`, `${testPath}/${testName}`,
        `${testPath}/${testName}`, `${testPath}/${testName}`,
      ).join(';');
      expect(applyFunc(pathdata, testPath)).toEqual(correct);
    });
  });
});
